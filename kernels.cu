...
__device__ double getCurrentImpact(int i,int l ,int k,
		CurrentTensorComponent *t1)
{
    if(t1->i11 == i && t1->i12 == l && t1->i13 == k) return t1->t[0];
    if(t1->i21 == i && t1->i22 == l && t1->i23 == k) return t1->t[1];
    if(t1->i31 == i && t1->i32 == l && t1->i33 == k) return t1->t[2];
    if(t1->i41 == i && t1->i42 == l && t1->i43 == k) return t1->t[3];

    return 0.0;
}

__device__ double getCurrentImpact2(int i,int l ,int k,
		CurrentTensorComponent *t1,CurrentTensorComponent *t2,int pqr2)
{
	return getCurrentImpact(i,l,k,t1) + ((pqr2 == 2) ? 1.0 : 0.0 )*getCurrentImpact(i,l,k,t2);
}
...
__device__ void assignSharedWithLocal3(
		                         CellDouble **c_jx,
		                         CellDouble **c_jy,
		                         CellDouble **c_jz,

		                         CellDouble *fdx,CellDouble *fdy,CellDouble *fdz)
{
...
__device__ void copyFieldsToSharedMemory3(
		 CellDouble *c_jx,
		 CellDouble *c_jy,
		 CellDouble *c_jz,
		 CellDouble *sc_jx,
		...
		)
{
	...
	while(index < CellExtent*CellExtent*CellExtent)
	{
		...
		copyCellDouble(c_jx,sc_jx,index,blockId);
		copyCellDouble(c_jy,sc_jy,index,blockId);
		copyCellDouble(c_jz,sc_jz,index,blockId);
		}
		index += blockDimX;
	}
...
__device__ void loop_copyCellDouble(
		 CellDouble *c_jx,
		 CellDouble *sc_jx,
		 Cell *c,
		 int index,
		...
		)
{
	...
	if(threadIdx.x < CellExtent*CellExtent*CellExtent)
	{
	        k = threadIdx.x % CellExtent;
			l = (threadIdx.x / CellExtent) % CellExtent;
			i = (threadIdx.x / CellExtent) / CellExtent;
//			m_c_jx->M[i][l][k] = 0.0;
			for(int i = 0;i < size;i++)
			{
			    CellDouble *cdx  = &(m_c_jx[i]);
			    cdx->M[i][l][k] = 0.0;
			    CellDouble *cdy  = &(m_c_jy[i]);
			    cdy->M[i][l][k] = 0.0;
			    CellDouble *cdz  = &(m_c_jz[1]);
			    cdz->M[i][l][k] = 0.0;
			}
	}
}

	__device__ void add_cell_double_threads(
			 CellDouble *a,
			 CellDouble *b,
			// int size,
			 Cell *c,
...
//                		&& (i == 2) && (l == 2) && (k == 2))
                {
                	printf("add %d %d %d a %10.3e b %10.3e res %10.3e\n",i,l,k,t,b->M[i][l][k],a->M[i][l][k]);
                }
...
//        double q = c_jx->M[2][2][2],q1 = c_jx1->M[2][2][2],a;
...
//        else
//        {
//           writeCurrentComponent(c_jx1,&(dt.t1.Jx),&(dt.t2.Jx),pqr2);
//        }
////        if((c->i == 28) && (c->l == 3) && (c->k == 2) && (nt == 1))
////        {
////        	a_c_jx_222[index] = c_jx->M[2][2][2];
////        	add_222[index] = getCurrentImpact2(2,2,2,&(dt.t1.Jx),&(dt.t2.Jx),pqr2);
////        }
...
//    while(index < c->number_of_particles && index > c->number_of_particles/2)
//        {
//            c->AccumulateCurrentSingleParticle    (index,&pqr2,&dt);
//
//
...
////            if((c->i == 28) && (c->l == 3) && (c->k == 2) && (nt == 1))
////            {
////            	if((a = getCurrentImpact2(2,2,2,&(dt.t1.Jx),&(dt.t2.Jx),pqr2)) > 0)
////            	{
////            		printf("impact index %5d c_jx %10.3e c_jx1 %10.3e adding %10.3e \n",
////            				index,
////            				fabs(q-c_jx->M[2][2][2]),
////            				fabs(q1-c_jx1->M[2][2][2]),
////            				a);
////            	}
////            }
//
		                         ...
		                         )
{
	...
	__shared__ CellDouble fdx[CURRENT_SUM_BUFFER_LENGTH];
	__shared__ CellDouble fdy[CURRENT_SUM_BUFFER_LENGTH];
	...
	assignSharedWithLocal3(&c_jx,&c_jy,&c_jz,fdx,fdy,fdz);

	set_cell_double_arrays_to_zero(fdx,fdy,fdz,CURRENT_SUM_BUFFER_LENGTH,
   ...
   // int index = threadIdx.x + blockDim.x*threadIdx.y;
	...
	AccumulateCurrentWithParticlesInCell(&(fdx[0]),c_jy,c_jz,
							 ...
							 b_c_jx_222,a_c_jx_222,add_222);


//    __syncthreads();
	if((c->i == 28) && (c->l == 3) && (c->k == 2) && (nt == 1))
    {
 	   loop_copyCellDouble(&(fdx_copy[0]),&(fdx[0]),c,threadIdx.x,blockIdx,blockDim.x);
	   loop_copyCellDouble(&(fdx_copy[1]),&(fdx[1]),c,threadIdx.x,blockIdx,blockDim.x);
	   loop_copyCellDouble(&(fdx_copy[2]),&(fdx[2]),c,threadIdx.x,blockIdx,blockDim.x);
	}
	...
	add_cell_double_threads(c_jx,&(fdx[1]),c,threadIdx,blockIdx,blockDim,nt);
//	add_cell_double_threads(c_jy,m_c_jy,threadIdx,blockIdx,blockDim);
//	add_cell_double_threads(c_jz,m_c_jz,threadIdx,blockIdx,blockDim);
    ...
    if((c->i == 28) && (c->l == 3) && (c->k == 2) && (nt == 1))
        {
    	   loop_copyCellDouble(c_jx_a,c_jx,c,threadIdx.x,blockIdx,blockDim.x);
        }

    if((c->i == 28) && (c->l == 3) && (c->k == 2) && (nt == 1))
    {
    	add_222[MAX_particles_per_cell-1] = c->number_of_particles;
    }
