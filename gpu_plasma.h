...
//                mass,q_mass };
	   ...
	   double *d_b_c_jx_222,*d_a_c_jx_222,*d_add_222;
	   double *b_c_jx_222,*a_c_jx_222,*add_222;
	   CellDouble cx_b,cx_a,fdx[CURRENT_SUM_BUFFER_LENGTH];
	   CellDouble  *d_a,*d_b,*d_fdx;

	   MemoryAllocate((void **)&d_b_c_jx_222,sizeof(double)*MAX_particles_per_cell);
	   MemoryAllocate((void **)&d_a_c_jx_222,sizeof(double)*MAX_particles_per_cell);
	   MemoryAllocate((void **)&d_add_222,sizeof(double)*MAX_particles_per_cell);

	   MemoryAllocate((void **)&d_a,sizeof(CellDouble));
	   MemoryAllocate((void **)&d_b,sizeof(CellDouble));
	   MemoryAllocate((void **)&d_fdx,sizeof(CellDouble)*CURRENT_SUM_BUFFER_LENGTH);



	   b_c_jx_222 = (double *)malloc(sizeof(double)*MAX_particles_per_cell);
	   a_c_jx_222 = (double *)malloc(sizeof(double)*MAX_particles_per_cell);
	   add_222    = (double *)malloc(sizeof(double)*MAX_particles_per_cell);


	   dim3 dimBlock1(256,2,1);
	   void *args1[] = { (void* )&d_CellArray,&nt,&d_b_c_jx_222,&d_a_c_jx_222,&d_add_222,&d_b,&d_fdx,&d_a,0};
//	   GPU_CurrentsAllCells//<<<dimGrid, dimBlock,16000>>>(d_CellArray);//,0,d_Jx,
	   ...
	   MemoryCopy(b_c_jx_222,d_b_c_jx_222,sizeof(double)*MAX_particles_per_cell,DEVICE_TO_HOST);
	   MemoryCopy(a_c_jx_222,d_a_c_jx_222,sizeof(double)*MAX_particles_per_cell,DEVICE_TO_HOST);
	   MemoryCopy(add_222,d_add_222,sizeof(double)*MAX_particles_per_cell,DEVICE_TO_HOST);


	   MemoryCopy(&cx_b,d_b,sizeof(CellDouble),DEVICE_TO_HOST);
	   MemoryCopy(&cx_a,d_a,sizeof(CellDouble),DEVICE_TO_HOST);
	   MemoryCopy(&fdx,d_fdx,sizeof(CellDouble)*CURRENT_SUM_BUFFER_LENGTH,DEVICE_TO_HOST);



       int nump = (int)add_222[MAX_particles_per_cell-1];
//	   for(int i = 0; i< nump;i++)
//	   {
//		   printf("28_3_2 index %5d before %10.3e after %10.3e add %e \n",i,b_c_jx_222[i],a_c_jx_222[i],add_222[i]);
//	   }
       ...
       if(nt == 1)
       {
			   for(int i = 0;i < CellExtent;i++)
			   {
				   for(int l = 0;l < CellExtent;l++)
				   {
					   ...
					   {
						   printf("28_3_2 %d %d %d before %10.3e after %10.3e fdx ",i,l,k,cx_b.M[i][l][k],cx_a.M[i][l][k]);
						   for(int n = 0;n < CURRENT_SUM_BUFFER_LENGTH;n++) printf(" %10.3e ",fdx[n].M[i][l][k]);
						   printf("\n");
